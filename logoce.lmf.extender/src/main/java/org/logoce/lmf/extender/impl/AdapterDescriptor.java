...
import org.logoce.lmf.extender.api.parameter.IParameterResolver;
import org.logoce.lmf.extender.api.reflect.ConstructorHandle;
import org.logoce.lmf.extender.ext.IAdaptableNameMatcher;
import org.logoce.lmf.extender.impl.util.AnnotationHandlesBuilder;
import org.logoce.lmf.extender.impl.util.ExtenderBuilder;
...
public final class AdapterDescriptor<Extender extends IAdapter> implements IAdapterDescriptor<Extender>
{
												 ...
												 final Stream<? extends IParameterResolver> resolvers) throws ReflectiveOperationException
	{
		...
		final var annotationHandles = executionHandleBuilders.stream().map(builder -> builder.build(extender)).toList();
	...
	public boolean isApplicable(final IAdaptable target)
	{
		...
		{
			return nameMatchers.stream().anyMatch(m -> m.match(target, annotation.name()));
		}
	...
	{
		return Arrays.stream(rootClass.getInterfaces()).anyMatch(i -> i == _interface);
	}
