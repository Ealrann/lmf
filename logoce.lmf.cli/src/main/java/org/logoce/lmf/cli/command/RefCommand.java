...
public final class RefCommand implements Command
{
	...
	public RefCommand(final String modelSpec, final String targetReference, final boolean includeDescendants)
	{
	...
	public static RefCommand parse(final List<String> args, final PrintWriter err)
	{
		...
		for (int i = 0; i < args.size(); i++)
		{
			...
			if (arg != null && arg.startsWith("--"))
			{
				...
				err.println("Usage: lm [--project-root <path>] ref <model.lm> <ref> [--include-descendants]");
				return null;
			}
			if (modelSpec == null)
			{
		...
		return new RefCommand(modelSpec, targetRef, includeDescendants);
	}
	...
	public int execute(final CliContext context)
	{
		...
		return runner.run(context, modelSpec, targetReference, new RefRunner.Options(includeDescendants));
	}
}
