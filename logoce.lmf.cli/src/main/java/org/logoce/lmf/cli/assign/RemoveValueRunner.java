...
public final class RemoveValueRunner
{
				   ...
				   final Options options)
	{
		...
		final var resolved = ModelSpecResolver.resolve(context, modelSpec, "remove-value", options.json());
		if (!resolved.ok())
		{
			return resolved.exitCode();
		}
		return removeValueInModel(resolved.path(), context, modelSpec, objectReference, featureName, value, options);
	}

	private int removeValueInModel(final Path modelPath,
								   ...
								   final Options options)
	{
		...
		if (documents == null)
		{
			if (options.json())
			{
				JsonErrorWriter.writeError(context, "remove-value", ExitCodes.INVALID, "Cannot load workspace documents for " + displayPath);
			}
			else
			{
				err.println("No changes written to " + displayPath);
		...
		if (linkRoots.isEmpty())
		{
			...
			if (options.json())
			{
				JsonErrorWriter.writeError(context, "remove-value", ExitCodes.INVALID, "No link trees available for " + displayPath);
			}
			return ExitCodes.INVALID;
		}
		...
		if (!(resolved instanceof RootReferenceResolver.Resolution.Found found))
		{
			...
			if (options.json())
			{
				JsonErrorWriter.writeError(context, "remove-value", ExitCodes.USAGE, message);
			}
			else
			{
				err.println(message);
		...
		try
		{
			edit = planner.planRemoveValue(found.node(), targetDoc.document().source(), featureName, value);
		}
		catch (RuntimeException e)
		{
			...
			if (options.json())
			{
				JsonErrorWriter.writeError(context, "remove-value", ExitCodes.INVALID, message);
			}
			else
			{
				err.println(message);
		...
		if (edit == null)
		{
			...
			{
				context.out().println("OK: nothing to remove from " + featureName + " on " + objectReference + " in " + displayPath);
			}
			return ExitCodes.OK;
		}
		...
		if (options.json())
		{
			writeJsonResult(context,
							...
							value,
							outcome,
							"OK: removed value from " + featureName,
							ExitCodes.OK);
		}
		else
		{
			context.out().println("OK: removed value from " + featureName + " on " + objectReference + " in " + displayPath);
		}
		return ExitCodes.OK;
	}
										...
										final int exitCode)
	{
		...
		json.beginObject()
			.name("command").value("remove-value")
