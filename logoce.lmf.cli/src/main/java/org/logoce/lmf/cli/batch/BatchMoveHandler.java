...
final class BatchMoveHandler implements BatchOperationHandler
{
	private final BatchOperationSupport support;

	BatchMoveHandler(final BatchOperationSupport support)
	{
	...
	public BatchOperationResult execute(final BatchOperation operation)
	{
		...
		if (parsed == null)
		{
			return support.invalidArgs(operation, options, "move", "<model.lm> <fromRef> <toRef>");
		}
		...
		{
			context.out().println(support.opPrefix(operation) + "OK: nothing to move");
			return support.result(operation,
							  ExitCodes.OK,
							  false,
							  "noop",
		...
		context.out().println(support.opPrefix(operation) + "STAGED: moved " + fromReference + " to " + toReference);
		return support.result(operation,
						  ExitCodes.OK,
						  !validated,
						  "staged",
						  "Moved " + fromReference + " to " + toReference,
