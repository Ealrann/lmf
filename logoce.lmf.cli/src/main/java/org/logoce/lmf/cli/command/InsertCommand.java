...
public final class InsertCommand implements Command
{
	...
	public static InsertCommand parse(final List<String> args, final PrintWriter err)
	{
		...
		return new InsertCommand(modelSpec, targetReference, subtreeSource, json);
	}
	...
	public int execute(final CliContext context)
	{
		...
		if (read instanceof SubtreeSourceReader.ReadResult.Failure failure)
		{
			if (json)
			{
				JsonErrorWriter.writeError(context, "insert", failure.exitCode(), failure.message());
			}
			else
			{
		...
		final var subtree = ((SubtreeSourceReader.ReadResult.Success) read).subtree();
		final var runner = new InsertRunner();
		return runner.run(context, modelSpec, targetReference, subtree, new InsertRunner.Options(json));
	}

	private static void printUsage(final PrintWriter err)
	{
		err.println("Usage: lm [--project-root <path>] insert <model.lm> <ref> --subtree-file <path> [--json]");
		err.println("   or: lm [--project-root <path>] insert <model.lm> <ref> --subtree-stdin [--json]");
		err.println("   or: lm [--project-root <path>] insert <model.lm> <ref> - [--json]");
		err.println("   or: lm [--project-root <path>] insert <model.lm> <ref> <subtree> [--json]   (deprecated; one-liner only)");
	}
}
