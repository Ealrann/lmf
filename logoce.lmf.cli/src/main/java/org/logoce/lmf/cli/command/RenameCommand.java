...
public final class RenameCommand implements Command
{
	...
	public static RenameCommand parse(final List<String> args, final PrintWriter err)
	{
		if (args.isEmpty())
		{
			err.println("Usage: lm [--project-root <path>] rename <model.lm> <ref> <newName>");
			return null;
		}
		...
		return new RenameCommand(modelSpec, ref, newName);
	}

	@Override
	public int execute(final CliContext context)
	{
		return new RenameRunner().run(context, modelSpec, objectReference, newName);
	}
}
