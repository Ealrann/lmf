...
final class BatchInsertHandler implements BatchOperationHandler
{
	private final BatchOperationSupport support;

	BatchInsertHandler(final BatchOperationSupport support)
	{
	...
	public BatchOperationResult execute(final BatchOperation operation)
	{
		...
		if (parsed == null)
		{
			return support.invalidArgs(operation, options, "insert", "<model.lm> <ref> <subtree>");
		}
		...
		final var subtreeFormat = support.formatSingleRootSubtree(subtree, context.err(), operation);
		if (subtreeFormat instanceof BatchOperationSupport.SubtreeFormatResult.Failure failure)
		{
		...
		final var formattedSubtree = ((BatchOperationSupport.SubtreeFormatResult.Success) subtreeFormat).formatted();
		...
		context.out().println(support.opPrefix(operation) + "STAGED: inserted into " + targetReference);
		return support.result(operation,
						  ExitCodes.OK,
						  !validated,
						  "staged",
						  "Inserted into " + targetReference,
