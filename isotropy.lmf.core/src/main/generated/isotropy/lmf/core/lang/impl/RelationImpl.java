...
public final class RelationImpl<UnaryType extends LMObject, EffectiveType> extends FeaturedObject implements Relation<UnaryType, EffectiveType> {
  private static final FeatureGetter<Relation<?, ?>> GET_MAP = new FeatureGetter.Builder<Relation<?, ?>>().add(Features.name, Relation::name).add(Features.immutable, Relation::immutable).add(Features.many, Relation::many).add(Features.mandatory, Relation::mandatory).add(Features.rawFeature, Relation::rawFeature).add(Features.reference, Relation::reference).add(Features.lazy, Relation::lazy).add(Features.contains, Relation::contains).build();
