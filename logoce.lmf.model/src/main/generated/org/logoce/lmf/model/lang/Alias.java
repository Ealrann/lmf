...
public interface Alias extends Named {
  ...
  interface Features<T extends Features<T>> extends Named.Features<T> {
    ...
    Attribute<String, String> VALUE = new AttributeBuilder<String, String>().name("value").immutable(true).mandatory(true).id(Alias.FeatureIDs.VALUE).datatype(() -> LMCoreModelDefinition.Units.STRING).build();
